...
class LoanViewModel(private val application: Application): AndroidViewModel(application) {
    ...
    fun loanSearch(
        loanerName: String?,
        amountStartString: String?,
        amountEndString: String?,
        ...
        try {
            ...
            val loanerNameChecked : String? =
                if (loanerName.isNullOrEmpty()) null else loanerName
            val searchTermStringChecked : String? =
                if (searchTermString.isNullOrEmpty()) null else searchTermString
            ...
            _inSearching.postValue(true)  //set _inSearching to false for trigger activity observer
        } catch (e:Exception){
    ...
    fun loanUpdate(
        ...
        try {
            if (
                loanerName.isEmpty() || amountString.isEmpty() || dateString.isEmpty() || note.isEmpty()){
                _insertFieldsIsEmpty.postValue(true) //maybe can add witch field is blank message
            } else {
                val isPaidBoolean = isPaidInt==0
                val amountFloat = amountString.toFloat()
                val dateLong = SimpleDateFormat("yyyy/MM/dd").parse(dateString).time
                val loan2Update = LoanEntity(
                    loanId = loanId,
                    loanerName = loanerName,
                    amount = amountFloat,
                    date = dateLong,
